...
# Federation rework by KazukoBot 2021
...
def join_fed(update: Update, context: CallbackContext):
    ...
    if len(args) >= 1:
        ...
        if not x:
            message.reply_text(
                "Failed to join federation! Please contact @CFC_BOT_support should this problem persist!"
            )
            return
...
def fed_help(update: Update, context: CallbackContext):
    ...
    if help_info == "owner":
        ...
        help_text =(pdate.effective_chat.id, "FED_USER_HELP") 
    query.message.edit_text(
        text=help_text,
        parse_mode=ParseMode.MARKDOWN,
        reply_markup=InlineKeyboardMarkup(
...
"""
    ...
    [
        InlineKeyboardButton(text="Any users", callback_data="kazuko_users_help",
        ),
